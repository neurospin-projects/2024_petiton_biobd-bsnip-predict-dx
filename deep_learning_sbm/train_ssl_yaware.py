...
from new_ssl_models import simCLR
from yaware_loss import GeneralizedSupervisedNTXenLoss

"""
Same as train_ssl.py with added yaware_loss instead of the usual SimCLR NT-Xent loss.

train_ssl.py can be found here: https://github.com/neurospin-projects/2022_cambroise_surfaugment/tree/master 
...
parser.add_argument(
    "--sigma_yaware", type=int, default=5,
    help="sigma y aware"
)
...
loss_fn = GeneralizedSupervisedNTXenLoss(
    temperature=args.loss_param,
    sigma=args.sigma_yaware,  
    return_logits=True
)
...
checkpoint_path_encoder = os.path.join(checkpoint_dir, "model_epoch_{}_encoder.pth")
...
for epoch in range(start_epoch, args.epochs + 1):
    ...
    for step, data in enumerate(loader, start=epoch * len(loader)):
        ...
        with torch.amp.autocast(device_type=device_type):
            ...
            # with : 
            z_i, z_j = model.forward(*forwarded)
            labels = metadata["age"].float().to(device)
    ...
    if epoch % args.save_freq == 0:
        ...
        torch.save(model.backbone.state_dict(), checkpoint_path_encoder.format(epoch))

...
def classif_vbm_ROI(classif='svm', datasize=800, save = False, N763=False, compute_shap=False, atlas="Neuromorphometrics"):
    ...
    for site in get_predict_sites_list():
        ...
        if compute_shap :
            ...
            print("background_data", np.shape(background_data),background_data)
            ...
            save_pkl(shap_values,RESULTSFOLDER+"shap/ShapValues_VBM_SVM_RBF_"+site+"_background_alltr_parallelized.pkl")
            # if showplot : shap.summary_plot(shap_values, X_test, feature_names=[f'Feature {i}' for i in range(X_test.shape[1])])
    ...
    if save and classif=="svm" and not N763:
...
def main():
    ...
    # classif_vbm_ROI(classif = "svm", datasize = 800, save = False, N763=False, compute_shap=True)
